\echo '005_profession_04_mage.sql'
-- Вариант ответа: Маг
...
-- Базовые правила видимости (профессия "Маг" только для расы Human или Elf)
, rule_parts AS (
  SELECT
...
, vis_rules AS (
  ...
  SELECT
    ck_id('witcher_cc.rules.wcc_profession.mage') AS ru_id,
    'wcc_profession_mage' AS name,
    jsonb_build_object(
      'or',
      jsonb_build_array(
...
SELECT 'wcc_profession_o' || to_char(raw_data.num, 'FM00') AS an_id,
       ...
       ck_id('witcher_cc.rules.wcc_profession.mage') AS visible_ru_ru_id,
       jsonb_build_object(
...
SELECT
  ...
  jsonb_build_object(
    ...
    jsonb_build_array(
      ...
      jsonb_build_object(
        'tokens', 5,
        'items', jsonb_build_array('T900', 'T012', 'W082', 'T020', 'T048', 'T091', 'T060', 'T049', 'W157', 'T115'),
...
WITH skill_mapping (skill_name) AS ( VALUES
    ...
    ('resist_magic'),         -- Сопротивление магии
...
SELECT
  ...
  jsonb_build_object(
    ...
    jsonb_build_array(
      ...
      jsonb_build_object('id', 'magical_training', 'name', ck_id('witcher_cc.wcc_skills.magical_training.name')::text)
    )
  ) AS body;
...
, ins_profession AS (
  INSERT INTO i18n_text (id, entity, entity_field, lang, text)
  SELECT ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'.'|| 'o04' ||'.'|| meta.entity ||'.'|| 'profession') AS id
...
SELECT
  ...
  jsonb_build_object(
    ...
    jsonb_build_array(
      ...
      jsonb_build_object('i18n_uuid', ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'.'|| 'o04' ||'.'|| meta.entity ||'.'|| 'profession')::text)
    )
  ) AS body
FROM meta
UNION ALL
