\echo '005_profession_08_craftsman.sql'
...
WITH prof_skill_mapping (skill_id, branch_number, professional_number) AS ( VALUES
  ...
  ('apprentice', 1, 2),
  ('masterwork', 1, 3),
  ('alchemical_concoction', 2, 1),
  ('enhanced_potion', 2, 2),
  ('experimental_formula', 2, 3),
  ('workshop', 3, 1),
  ('repair', 3, 2),
  ('upgrade', 3, 3)
)
...
SELECT
  ...
  jsonb_build_object(
    ...
    jsonb_build_array(
      ...
      jsonb_build_object('id', 'quick_fix', 'name', ck_id('witcher_cc.wcc_skills.quick_fix.name')::text)
    )
  ) AS body;
...
, ins_profession AS (
  INSERT INTO i18n_text (id, entity, entity_field, lang, text)
  SELECT ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'.'|| 'o08' ||'.'|| meta.entity ||'.'|| 'profession') AS id
...
SELECT
  ...
  jsonb_build_object(
    ...
    jsonb_build_array(
      ...
      jsonb_build_object('i18n_uuid', ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'.'|| 'o08' ||'.'|| meta.entity ||'.'|| 'profession')::text)
    )
  ) AS body
FROM meta
UNION ALL
