\echo '032_life_events_fortune_or_not_details_curse.sql'
...
, ins_body AS (
    ...
    SELECT ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'.'|| meta.entity ||'.'|| 'body') AS id
      ...
      FROM (VALUES
        ('ru', 'Придумайте свой вариант проклятия для персонажа.'),
...
-- i18n для описания "Кастомное проклятие"
, ins_event_desc AS (
  INSERT INTO i18n_text (id, entity, entity_field, lang, text)
    SELECT ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'.'|| 'event_desc') AS id
         , meta.entity, 'event_desc', v.lang, v.text
      FROM (VALUES
        ('ru', 'Кастомное проклятие'),
        ('en', 'Custom Curse')
      ) AS v(lang, text)
      CROSS JOIN meta
)
-- Эффекты: добавление кастомного проклятия в массив diseases_and_curses и в lifeEvents
