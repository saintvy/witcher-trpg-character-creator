\echo '082_witcher_events_hunt_location.sql'
...
, ins_body AS (
    ...
    SELECT ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'.'|| meta.entity ||'.'|| 'body') AS id
      ...
      FROM (VALUES
        ('ru', 'Определите место, где находилась цель охоты.'),
...
, raw_data AS (
  SELECT 'ru' AS lang, raw_data_ru.* FROM (VALUES
    (1, 0.1, 'Лес'),
    (2, 0.1, 'Здание'),
    (3, 0.1, 'Заброшенное здание'),
    (4, 0.1, 'Морское побережье'),
    (5, 0.1, 'Горы'),
    (6, 0.1, 'Город'),
    (7, 0.1, 'Кладбище'),
    (8, 0.1, 'Деревня'),
...
-- Связь: напрямую из ноды цели охоты
