\echo '005_profession_05_man_at_arms.sql'
-- Вариант ответа: Воин
...
SELECT
  ...
  jsonb_build_object(
    ...
    jsonb_build_array(
      ...
      jsonb_build_object('id', 'tough_as_nails', 'name', ck_id('witcher_cc.wcc_skills.tough_as_nails.name')::text)
    )
  ) AS body;
...
, ins_profession AS (
  INSERT INTO i18n_text (id, entity, entity_field, lang, text)
  SELECT ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'.'|| 'o05' ||'.'|| meta.entity ||'.'|| 'profession') AS id
...
SELECT
  ...
  jsonb_build_object(
    ...
    jsonb_build_array(
      ...
      jsonb_build_object('i18n_uuid', ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'.'|| 'o05' ||'.'|| meta.entity ||'.'|| 'profession')::text)
    )
  ) AS body
FROM meta
UNION ALL
