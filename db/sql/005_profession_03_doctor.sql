\echo '005_profession_03_doctor.sql'
...
-- Базовые правила видимости (профессия "Медик" только для расы НЕ ведьмак)
, rule_parts AS (
  SELECT
...
, vis_rules AS (
  ...
  SELECT
    ck_id('witcher_cc.rules.wcc_profession.doctor') AS ru_id,
    'wcc_profession_doctor' AS name,
...
SELECT 'wcc_profession_o' || to_char(raw_data.num, 'FM00') AS an_id,
       ...
       ck_id('witcher_cc.rules.wcc_profession.doctor') AS visible_ru_ru_id,
       jsonb_build_object(
...
WITH prof_skill_mapping (skill_id, branch_number, professional_number) AS ( VALUES
  ('diagnose', 1, 1),
  ('analysis', 1, 2),
  ('effective_surgery', 1, 3),
  ('healing_tent', 2, 1),
  ('improvised_medicine', 2, 2),
  ('herbal_remedy', 2, 3),
  ('bloody_wound', 3, 1),
  ('practical_butchery', 3, 2),
  ('crippling_wound', 3, 3)
)
INSERT INTO effects (scope, an_an_id, body)
SELECT
...
SELECT
  ...
  jsonb_build_object(
    ...
    jsonb_build_array(
      ...
      jsonb_build_object('id', 'healing_hands', 'name', ck_id('witcher_cc.wcc_skills.healing_hands.name')::text)
    )
  ) AS body;
...
, ins_profession AS (
  INSERT INTO i18n_text (id, entity, entity_field, lang, text)
  SELECT ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'.'|| 'o03' ||'.'|| meta.entity ||'.'|| 'profession') AS id
...
SELECT
  ...
  jsonb_build_object(
    ...
    jsonb_build_array(
      ...
      jsonb_build_object('i18n_uuid', ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'.'|| 'o03' ||'.'|| meta.entity ||'.'|| 'profession')::text)
    )
  ) AS body
FROM meta
UNION ALL
