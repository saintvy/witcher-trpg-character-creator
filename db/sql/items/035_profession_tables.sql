  ...
  raw_prof_skills AS (
    SELECT 'ru' AS lang, raw_data_ru.*
      FROM (VALUES
        ...
        -- Priest branch 2 (DLC exp_toc): Культист
        (291, 'mystagogue', 'Мистагог', 'emp', 'professional', 1, 2, 'Культист', false, 'priest'),
        (292, 'cult_mystery', 'Тайна культа', 'will', 'professional', 2, 2, 'Культист', false, 'priest'),
        (293, 'blessings', 'Благословения', 'will', 'professional', 3, 2, 'Культист', false, 'priest'),
        ...
        (254, 'holy_fire', 'Слово божье', 'emp', 'professional', 3, 3, 'Фанатик', false, 'priest'),
    ...
    SELECT 'en' AS lang, raw_data_en.*
      FROM (VALUES
        ...
        (248, 'foresight', 'Precognition', 'will', 'professional', 3, 1, 'The Preacher', false, 'priest'),
        ...
        (254, 'holy_fire', 'Word of God', 'emp', 'professional', 3, 3, 'The Fanatic', false, 'priest'),
  ...
  raw_prof_skill_descriptions AS (
    SELECT 'ru' AS lang, skill_id, description
      FROM (VALUES
        ...
        ('mystagogue', 'Будучи в поселении, жрец, потратив день и пройдя проверку способности <strong>Мистагог</strong> со СЛ в зависимости от поселения, может построить святилище своего божества, которое привлекает людей и обращает их в свою религию. Количество единомышленников определяет ведущий. К тому же, находясь в пределах 20 м от этого святилища, жрец может использовать это святилище в качестве фокусирующего предмета со значением 3.'),
        ('cult_mystery', 'На более высоких уровнях жрецу показывают внутреннюю работу его религии и посвящают в некоторые её секреты. <strong>Тайна культа</strong> жрецов зависит от типа религии, которой они следуют. Обратитесь к таблице Тайны культа, чтобы узнать, какая из них.'),
        ('blessings', 'Жрец может благословить группу людей, проведя часовую церемонию. В конце церемонии он проходит проверку навыка <strong>Благословения</strong> со СЛ 16. В течение одного дня после этого число людей, равное уровню <strong>Благословения</strong> жреца, получает бонусы в соответствии с таблицей Благословений.'),
    ...
    SELECT 'en' AS lang, skill_id, description
      FROM (VALUES
        ...
        ('mystagogue', 'When in a settlement, a Priest can spend a day and roll <strong>Mystagogue</strong> at a DC based on the settlement to build a shrine to their deity that attracts people and converts them to their religion. The GM determines how many people join. Additionally, while within 20m of this shrine, the Priest can use the shrine as a Focus with a value of 3.'),
        ('cult_mystery', 'At higher levels, a Priest is shown the inner workings of their religion and are privy to some of its secrets. The Priest''s cult mystery depends on the type of religion they follow. Refer to the Cult Mystery table to know which one.'),
        ('blessings', 'A Priest can bless a group of people through a 1-hour ceremony. At the end of the ceremony, they roll <strong>Blessings</strong> at a DC:16. For one day afterward, a number of people equal to the Priest''s <strong>Blessings</strong> level gain bonuses as per the Blessings table.'),
...
WHERE skill_id IN (
  ...
  'mystagogue', 'cult_mystery', 'blessings',
