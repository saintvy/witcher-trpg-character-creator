\echo '029_past_mentor_teaching_style.sql'

WITH
  meta AS (SELECT 'witcher_cc' AS su_su_id
                , 'wcc_past_mentor_teaching_style' AS qu_id
                , 'questions' AS entity)
, ins_body AS (
    ...
    SELECT ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'.'|| meta.entity ||'.body') AS id
      ...
      FROM (VALUES
              ('ru', 'Какой стиль преподавания был у наставника?'),
              ('en', 'What teaching style did your Mentor have?')
           ) AS v(lang, text)
      CROSS JOIN meta
...
, raw_data AS (
  SELECT 'ru' AS lang, raw_ru.*
    FROM (VALUES
            (1, 'Практичный', 1.0),
            (2, 'Терпеливый', 1.0),
            (3, 'Демонстративный', 1.0),
            (4, 'Небрежный', 1.0),
            (5, 'Властный', 1.0),
            (6, 'Манипулятивный', 1.0)
...
, ins_lore_i18n AS (
  INSERT INTO i18n_text (id, entity, entity_field, lang, text)
  SELECT ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'_o'|| to_char(raw_data.num, 'FM00') ||'.lore.mentor.teaching_style') AS id
       , 'character', 'mentor_teaching_style', raw_data.lang, raw_data.label_txt
    FROM raw_data
    CROSS JOIN meta
  ON CONFLICT (id, lang) DO NOTHING
)
...
WITH
  meta AS (SELECT 'witcher_cc' AS su_su_id
                , 'wcc_past_mentor_teaching_style' AS qu_id)
, nums AS (
    SELECT generate_series(1, 6) AS num
)
...
SELECT 'character'
     ...
     , jsonb_build_object(
         'set',
         jsonb_build_array(
           jsonb_build_object('var','characterRaw.lore.mentor.teaching_style'),
           jsonb_build_object(
             'i18n_uuid',
             ck_id(meta.su_su_id ||'.'|| meta.qu_id ||'_o'|| to_char(nums.num, 'FM00') ||'.lore.mentor.teaching_style')::text
           )
         )
       )
  FROM nums
